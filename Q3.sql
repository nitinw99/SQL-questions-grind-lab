...
CREATE or replace TABLE airbnb_apartments(host_id int,apartment_id varchar(5),apartment_type varchar(10),n_beds int,n_bedrooms int,country varchar(20),city varchar(20));
...
SELECT h.nationality AS nationality, COALESCE(SUM(N_BEDS),0) AS available_beds
FROM airbnb_apartments ap
JOIN airbnb_hosts h
ON ap.host_id = h.host_id
GROUP BY 1
