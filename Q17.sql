...
SELECT language,
COUNT(DISTINCT CASE WHEN device IN ('MacBook-Pro', 'iPhone 5s', 'iPad-air') THEN e.USER_ID END) AS apple_users,
COUNT(DISTINCT CASE WHEN device IS NOT NULL THEN e.USER_ID END) AS total_devices
