...
SELECT dr.trackname AS trackname, COUNT(dr.DATE) AS total_days
FROM spotify_daily_rankings_2017_us dr
JOIN spotify_worldwide_daily_song_ranking wr
ON dr.position=wr.position AND dr.trackname=wr.trackname AND TO_DATE(dr.date)=TO_DATE(wr.date)
WHERE dr.position=1 AND wr.position=1
GROUP BY 1;